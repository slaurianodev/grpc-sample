...
public class DepositService implements IGenericService {
    ...
    public int doProcess(OperationInput operationRequest, BalanceInput balanceRequest, String process) {
        ...
        if (process.equals("deposit")){
            initDAOs();
            return deposit(operationRequest);
        } else {
            return Response.UNRECOGNIZED.getNumber();
        }
    }

    private int deposit(OperationInput request){
        ...
        if(wallet != null){
            ...
            newWallet.setAmount(request.getAmount());
